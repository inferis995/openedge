...
func ParseAddress(c string) (Address, error) {
	...
	if val >= 40001 && val <= 49999 {
		return Address{Type: "holding", Offset: uint16(val - 40001), BitOffset: bitOffset}, nil
	}
	if val >= 30001 && val <= 39999 {
		return Address{Type: "input", Offset: uint16(val - 30001), BitOffset: bitOffset}, nil
	}
	if val >= 10001 && val <= 19999 {
		return Address{Type: "discrete", Offset: uint16(val - 10001)}, nil
	}
	if val >= 1 && val <= 9999 {
		// Coils can have bit offset if we treated them as registers, but standard 0xxxx are bits.
		// However, for consistency with PLC tags, 1 = Offset 0.
		return Address{Type: "coil", Offset: uint16(val - 1)}, nil
	}

	// Fallback for raw offsets (treated as holding for backward compatibility)
	// If it's a small number, we assume it's a raw holding register offset.
	if val < 30000 {
