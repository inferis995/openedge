...
const StatusBar = ({ data }: { data: NonNullable<ReturnType<typeof useDashboard>['data']> }) => {
    ...
    return (
        <div className="flex flex-wrap items-center gap-3 px-4 py-3 rounded-md border bg-card">
            {pill(sysOk, sysOk ? 'Sistema OK' : 'Sistema in errore')}
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border border-border bg-background">
                <Activity size={12} /> Uptime {formatDuration(data.system.api_uptime_sec)}
            </span>
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border ${
                crit > 0 ? 'border-red-500/30 text-red-500 bg-red-500/5'
                         : high > 0 ? 'border-orange-500/30 text-orange-500 bg-orange-500/5'
...
const KPICard = ({ k }: { k: KPIWidget }) => {
    // KPI con good_when='down' (es. allarmi/giorno) → ↑ è ROSSA.
    // KPI con good_when='up'  (es. login operatori)  → ↑ è VERDE.
    const isImprovement =
