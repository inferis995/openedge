...
func main() {
	...
	driver.alarmManager.OnAlarmEvent = func(eventID int, tagID int, alias string, def models.AlarmDefinition, val float64, status string) {
		log.Printf("[OPC-UA ALARM-EVENT] eventID=%d, tagID=%d, alias=%s, status=%s, severity=%s",
			eventID, tagID, alias, status, def.Severity)
...
func (d *Driver) publishAlarmEvent(eventID int, tagID int, alias string, def models.AlarmDefinition, val float64, status string) {
	...
	topic := fmt.Sprintf("sys/alarms/%d/%s/%s/%s/%s",
		cfg.OrgID, slugify(cfg.SiteName), slugify(cfg.AreaName),
		slugify(cfg.Gateway.Name), slugify(alias))
	...
	if err != nil {
		log.Printf("[OPC-UA Driver] Failed to encode alarm event for %s: %v", alias, err)
		return
	}
	...
	if err := d.mqttClient.PublishWithQoS(topic, string(payload), 1, false); err != nil {
		log.Printf("[OPC-UA Driver] Failed to publish alarm event for %s on %s: %v", alias, topic, err)
	}
}
